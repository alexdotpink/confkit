...
- `source().file('config.(json|yaml|yml|toml)')` — auto‑parsed by extension; missing/invalid → ignored. Also supports dotenv files like `.env`, `.env.local`, or any path ending in `.env`.

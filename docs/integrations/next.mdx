...
// Option A: process.env (Next injects from next.config)
function FromProcessEnv() {
  return (
    <>
      <div>App Name: {process.env.NEXT_PUBLIC_APP_NAME}</div>
      <div>Flag: {String(process.env.NEXT_PUBLIC_FEATURE_FLAG)}</div>
    </>
  );
}

// Option B: client module (typed via `confkit types`)
...
- TypeScript for client env: run `confkit types` to generate `confkit-env.d.ts` — it includes declarations for both `@confkit/next/client` and `confkit:client`.
